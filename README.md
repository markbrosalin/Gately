...
Клиентская часть Gately специально разделена на два движка.

Оба слоя построены модульно, но по-разному:

- UI-движок наиболее явно выражен как набор компонентов и сервисов,
- логический движок — как набор самостоятельных пакетов, runtime-модулей и расширяемого ядра.

### 1. Logic Engine

Логический движок отвечает за смысловую и вычислительную часть схемы:

- создание и управление вкладками, элементами и связями,
- хранение состояния схемы,
- выполнение симуляции,
- программируемое ядро движка,
- поддержку расширения через плагины.

В репозитории эта часть в основном живет в `packages/*`, прежде всего в:
...
Во фронтенде логический движок используется через worker-клиент. Это помогает отделить вычисление схемы от визуального слоя.

### 2. UI Engine

UI-движок отвечает за пользовательский редактор и визуальную инфраструктуру:

- управление workspace,
- отрисовку графа,
- построение визуальных узлов,
- синхронизацию состояния,
- работу с документами и каталогом,
- визуальное поведение графа и UI-плагины.

UI-движок не собран в один огромный файл. Он построен по компонентному принципу.
Ключевые части этой структуры:
...
Это важно, потому что проект становится легче развивать, тестировать и расширять.

### 3. Почему Это Разделение Вообще Важно

Для интервьюера или нового разработчика основная архитектурная мысль здесь такая:

- logic engine отвечает на вопрос "что схема означает и как она работает",
- UI engine отвечает на вопрос "как пользователь эту схему видит и редактирует".

Из-за этого Gately — это не просто UI-прототип.
Это попытка построить переиспользуемую систему, где вычислительный и визуальный слои намеренно развязаны.
